...
class Gui extends React.Component<{}, GuiState> {
	...
	render() {
		...
		const header = (
			<div className="page-header">
				...
				<p>
					How many insertions, deletions, and substitutions does it
					take to turn{" "}
					<input
						ref="str1"
						className="str"
				...
				<p>
					...
					{presets.map((p, i) => (
						<a
							key={i}
							onClick={() =>
								this.setState({ str1: p[0], str2: p[1] })
							}
						>
							<i>{p[0]}</i> and <i>{p[1]}</i>
							{i == presets.length - 1 ? "." : ", "}
		...
		if (matrix && levenpath) {
			...
			const steps = l.map((cur, i) => {
				let className =
					highlight.i == cur.i && highlight.j == cur.j
						? "highlighted"
						: "";
				const fragment = (() => {
					switch (cur.type) {
						case "replace":
							return (
								<span>
									<em>{str1.charAt(cur.i - 1)}</em> with{" "}
									<em>{str2.charAt(cur.j - 1)}</em>
								</span>
							);
						case "insert":
							return <em>{str2.charAt(cur.j - 1)}</em>;
						case "delete":
							return <em>{str1.charAt(cur.i - 1)}</em>;
						default:
							className += " grayed";
							return (
								<span className="grayed">
									don't change{" "}
									<em>{str1.charAt(cur.i - 1)}</em>
								</span>
							);
					}
				})();
				return (
					<li
						className={className}
						onMouseEnter={() =>
							this.setState({ highlight: { i: cur.i, j: cur.j } })
						}
						onMouseLeave={() =>
							this.setState({ highlight: { i: -1, j: -1 } })
						}
						key={cur.i + "," + cur.j}
					>
						<i>
							...
							<b>{str2[cur.j - 1]}</b>
							{str1.substr(cur.i)}
						</i>: {cur.type} {fragment} at position {cur.i}
					</li>
				);
			});
			result = (
				<div id="result">
					<table ref="table">
						...
						<tbody>
							{loop(str1.length + 1, i => (
								<tr key={i}>
									<th>{i > 0 ? str1.charAt(i - 1) : ""}</th>

									{loop(str2.length + 1, j => (
										<td
											key={j}
											style={{ opacity: 0 }}
											className={
					...
					<div ref="resultText">
						<p>
							...
							<strong>{matrix[str1.length][str2.length]}</strong>:
						</p>
						<ul>{steps}</ul>
						<label>
							<input
								type="checkbox"
								checked={this.state.showTrivial}
								onChange={e =>
									this.setState({
										showTrivial: (e.target as HTMLInputElement)
											.checked
									})
								}
							/>{" "}
	...
	componentDidMount() {
		const keyupspan = $(
			'<span class="str" style="width:auto;visibility:hidden;"></span>'
		);
		...
		$("input.str")
			.on("change", function() {
				...
				$(this)
					.stop(true)
					.animate({ "min-width": 8 + ~~keyupspan.width() }, 200);
			})
			.on("keypress", e => {
	...
	getTableCell(i: number, j: number): HTMLTableCellElement {
		return (this.refs["table"] as any).tBodies[0].children[i].children[
			j + 1
		];
	}
	componentWillUpdate(nextProps: any, nextState: GuiState) {
		if (
			nextState.str1 !== this.state.str1 ||
			nextState.str2 !== this.state.str2
		) {
			const ret = levenshtein(nextState.str1, nextState.str2);
			this.levenpath = ret.levenpath;
	...
	componentDidUpdate(prevProps: any, prevState: GuiState) {
		if (
			prevState.str1 !== this.state.str1 ||
			prevState.str2 !== this.state.str2
		) {
			for (let i = 0; i <= this.state.str1.length; i++)
				for (let j = 0; j <= this.state.str2.length; j++) {
					$(this.getTableCell(i, j))
						.css({ opacity: 0 })
						.delay(Math.max(i, j) * 200)
						.animate({ opacity: 1 }, 400);
				}
			// after all cells fade in
			$(this.refs["resultText"])
				.css("opacity", 0)
				.delay(
					Math.max(this.state.str1.length, this.state.str2.length) *
						200
				)
				.animate({ opacity: 1 }, 400);
		}
